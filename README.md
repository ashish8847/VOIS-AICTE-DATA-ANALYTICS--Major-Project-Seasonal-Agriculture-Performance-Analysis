...
The dataset covers farm records across Indian **states, districts, and crop types**, spanning the Kharif, Rabi, and Zaid seasons, with the following categories of features:
...
├── seasonal_agriculture_performance_dataset.csv      # Dataset (add if permitted to share)
...
2. Install dependencies:
   ...
   ```
3. Ensure `seasonal_agriculture_performance_dataset.csv` is in the project root (and that an `imgs/` folder exists for saved plots).
...
*This README was generated from the project notebook, PPT, and problem-statement brief. Update the GitHub link in the presentation and add the dataset (if licensing allows) before final submission.*
